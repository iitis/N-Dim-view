...
feature_cols = [
    'num_promises', 'promise_fulfillment', 'general_popularity',
    'sympathy', 'economic_views', 'social_views',
    'media_activity', 'voting_effectiveness', 'age'
]
label_col = 'group_numeric'
group_names = {0: "Populists", 1: "Conservatives", 2: "Liberals", 3: "Technocrats"}
...
for grp in sorted(df[label_col].unique()):
    axs[0,0].scatter(
        ...
        alpha=0.8, s=60, color=group_colors[grp]
    )
...
parallel_coordinates(pc_df[[label_col]+feature_cols], class_column=label_col, alpha=0.3, ax=axs[1,0], color=group_colors)
axs[1,0].set_title('Parallel coordinates')
axs[1,0].tick_params(axis='x', rotation=30, labelsize=9)
...
glyph_cols = ['promise_fulfillment','sympathy','economic_views','social_views','general_popularity']
...
axs[1,1].set_title("Chernoff faces (4x3 grid)")
axs[1,1].axis('off')
...
fig.suptitle('Comparison of classical visualization methods for the synthetic politicians dataset', fontsize=16)
plt.show()
